...
function createAuthRouter({ usersFile, readJSON, writeJSON, SECRET_KEY }) {
  ...
  router.post('/register', authLimiter, (req, res) => {
    ...
    if (!validateCredentials(username, password)) {
      return res.status(400).json({ error: { code: 'VALIDATION_ERROR', message: 'Invalid username or password format.' } });
    }
  ...
  router.post('/login', authLimiter, (req, res) => {
    ...
    if (!username || !password || username.trim() === '' || password.trim() === '') {
      return res.status(401).json({ error: { code: 'INVALID_INPUT', message: 'Username and password are required.' } });
    }
    
    // generated-by-copilot: Continue with normal validation
    if (!validateCredentials(username, password)) {
      return res.status(401).json({ error: { code: 'INVALID_CREDENTIALS', message: 'Invalid username or password format.' } });
    }
    try {
      const users = readJSON(usersFile);
