...
function createReviewsRouter({ reviewsFile, booksFile, readJSON, writeJSON, authenticateToken }) {
  ...
  router.post('/', authenticateToken, (req, res) => {
    ...
    if (!bookId || rating === undefined) {
      return res.status(400).json({ error: { code: 'VALIDATION_ERROR', message: 'bookId and rating are required' } });
    }
  ...
  router.delete('/:reviewId', authenticateToken, (req, res) => {
    ...
    try {
      ...
      if (index === -1) return res.status(404).json({ error: { code: 'REVIEW_NOT_FOUND', message: 'Review not found' } });
      if (reviews[index].username !== req.user.username) {
        return res.status(403).json({ error: { code: 'FORBIDDEN', message: 'Not authorized to delete this review' } });
      }
