...
Returns:
  ...
  async () => {
    ...
    for (const [field, def] of Object.entries(flagSchema)) {
      ...
      if (d.format) constraints.push(`format: ${d.format}`);
      if (d.pattern) constraints.push(`pattern: ${d.pattern}`);
      if (d.maxLength) constraints.push(`maxLength: ${d.maxLength}`);
      if (d.default !== undefined) constraints.push(`default: ${d.default}`);
      if (d.enum) constraints.push(`enum: ${(d.enum as string[]).join(', ')}`);

      lines.push(
        `| ${field} | ${d.type} | ${constraints.join('; ') || '—'} | ${d.description} |`
      );
    }
...
Returns:
  ...
  async () => {
    ...
    for (const flag of samples) {
      lines.push(
        `| ${flag.name} | ${flag.description} | ${flag.environment} | ${flag.enabled} |`
      );
    }
